...
static void search_engine_menu_popup_position_menu(GtkMenu *menu, gint *x, gint *y, gboolean *push_in, GtkWidget *widget)
{
...
void title_change(WebKitWebView *view, WebKitWebFrame *frame, const char *t, Browser *b)
{
	if (b->title)
		g_free(b->title);
	b->title = g_strdup(t);
